...
Insira uma fórmula DAX válida na_barra de fórmulas_. A barra de fórmulas inclui recursos como preenchimento automático, Intellisense e codificação de cores, que permitem que você insira a fórmula de forma rápida e precisa.
...
1. Na tabela`Salesperson`, faça a multisseleção das seguintes colunas e oculte-as (defina a propriedade**Está oculto** como_Sim_):
...
1. Na tabela`Date`, oculte a coluna`MonthKey` (defina**Está Oculto** como_Sim_).
...
1. No painel**Propriedades**, na caixa**Nome**, substitua o valor por_Fiscal_.
...
1. Na janela**Marcar como Tabela de Data**, deslize a propriedade**Marcar como Tabela de Data** para_Sim_.
...
1. Para a seleção de várias medidas, configure os seguintes requisitos:
    ...
    - Atribua a uma pasta de exibição chamada_Preços_ (use a propriedade da**Pasta de exibição** no painel**Propriedades**).
...
1. Faça a seleção múltipla das medidas`Order Lines` e`Orders` e configure os seguintes requisitos:
    ...
    - Atribuir a uma pasta de exibição chamada_Contagens_.
...
1. Renomeie a coluna`Targets | Target` como_TargetAmount_.
