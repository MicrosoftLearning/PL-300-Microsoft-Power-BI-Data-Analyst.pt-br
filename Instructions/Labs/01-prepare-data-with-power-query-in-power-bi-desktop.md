...
Este laboratório foi projetado para apresentá-lo ao aplicativo Power BI Desktop e como se conectar a dados e como usar técnicas de visualização de dados para entender as características e a qualidade dos dados de origem. Os objetivos de aprendizagem são:
...
Nesta tarefa, você começa abrindo um arquivo inicial do Power BI (.pbix). O arquivo inicial não contém dados, mas foi especialmente configurado para ajudá-lo a concluir o laboratório. As seguintes configurações em nível de relatório foram desabilitadas no arquivo inicial:
...
1. Na janela **Abrir**, procure a pasta **D:\DA100\Labs\load-data-with-power-query-in-power-bi-desktop\Starter**.
...
1. Na janela Banco de Dados SQL Server, na caixa Servidor, insira localhost.
...
1. No painel direito, observe uma visualização da tabela. A visualização permite que você determine as colunas e uma amostra de linhas.

1. Para criar consultas, selecione as seis seguintes tabelas:
...
1. Conclua esta tarefa clicando em **Transformar Dados**, que abrirá o Power Query Editor.
    1. *Este laboratório destina-se apenas a conectar e criar o perfil dos dados, mas não **a transformar dados**.*

## Formatar dados no Power Query Editor

Esta tarefa apresenta o Editor do Power Query e permite-lhe rever e criar o perfil dos dados. Isso ajuda a determinar como limpar e transformar os dados posteriormente.
...
1. Na parte inferior esquerda, na barra de status, observe as estatísticas da tabela: a tabela tem 33 colunas e 296 linhas.
...
1. Para avaliar a qualidade da coluna, na guia **Exibição** da faixa de opções, dentro do grupo **Visualização de Dados**, marque **Qualidade da Coluna**. A qualidade da coluna permite que você determine facilmente o percentual de valores válidos, com erro ou vazios.
...
1. Examine a distribuição de coluna para a coluna **EmployeeKey** (primeira): há 296 valores distintos e 296 valores exclusivos.
...
1. Examine a qualidade da coluna **TotalProductCost** e observe que 8% das linhas estão vazias.
    ...
    Para resolver o problema no laboratório Carregar dados no Power BI Desktop **, você aplicará transformações para preencher os valores ausentes usando o custo padrão do produto, que está armazenado na tabela **DimProduct.
...
1. Na janela **ResellerSalesTargets.csv**, examine a visualização de dados. Selecione **OK**.
...
1. Examine os ícones de cada cabeçalho de coluna à esquerda do nome da coluna. Os ícones representam o tipo de dados da coluna. **123** é número inteiro e **ABC** é texto.
     ...
     ![Figura 16](Linked_image_Files/01-prepare-data-with-power-query-in-power-bi-desktop_image38.png)

1. Use as etapas da tarefa anterior para criar uma consulta baseada no arquivo **D:\PL300\Resources\ColorFormats.csv**.
...
1. Na guia **Exibir** da faixa de opções, dentro do grupo **Visualizar Dados**, desmarque as três opções de visualização de dados:
    ...
    - Perfil da coluna
     ...
     ![Figura 48](Linked_image_Files/01-prepare-data-with-power-query-in-power-bi-desktop_image40.png)

1. Salve o arquivo do Power BI Desktop. Caso precise aplicar as alterações, selecione **Aplicar Mais Tarde**.
